...
export class SensoresService {
  ...
  async generarReporteSensores(scope: 'surco' | 'cultivo', scopeId: number, timeFilter: 'day' | 'date' | 'month', date?: string): Promise<Buffer> {
    // Obtener datos usando lógica similar a generateSensorReport
    let sensors: Sensor[] = [];
    let scopeName = '';
    ...
    for (const sensor of sensors) {
      ...
      if (sensor.valor_maximo_alerta && statistics.max > sensor.valor_maximo_alerta) {
        alertas.push(`Valor máximo excedido: ${statistics.max.toFixed(2)} > ${sensor.valor_maximo_alerta}`);
      }
      if (sensor.valor_minimo_alerta && statistics.min < sensor.valor_minimo_alerta) {
        alertas.push(`Valor mínimo excedido: ${statistics.min.toFixed(2)} < ${sensor.valor_minimo_alerta}`);
      }
  ...
  private generarHtmlReporteSensores(data: any): string {
    ...
    <body>
      ...
      <p><strong>Período:</strong> ${data.dateRange.start} - ${data.dateRange.end}</p>
      ...
      ${data.sensors.map(sensor => `
        <div class="sensor">
          ...
          <div class="stats">
            <div class="stat">
              <strong>Mínimo:</strong><br>${sensor.statistics.min.toFixed(2)}
            </div>
            <div class="stat">
              <strong>Máximo:</strong><br>${sensor.statistics.max.toFixed(2)}
            </div>
            <div class="stat">
              <strong>Promedio:</strong><br>${sensor.statistics.average.toFixed(2)}
            </div>
            <div class="stat">
              <strong>Desv. Estándar:</strong><br>${sensor.statistics.standardDeviation.toFixed(2)}
            </div>
          </div>
